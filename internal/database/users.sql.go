...
const setUserToChirpyRed = `-- name: SetUserToChirpyRed :one
UPDATE users
SET
    is_chirpy_red = TRUE,
    updated_at    = NOW()
...
type SetUserToChirpyRedRow struct {
	ID          uuid.UUID
	CreatedAt   time.Time
	UpdatedAt   time.Time
	Email       string
...
func (q *Queries) SetUserToChirpyRed(ctx context.Context, id uuid.UUID) (SetUserToChirpyRedRow, error) {
	row := q.db.QueryRowContext(ctx, setUserToChirpyRed, id)
	var i SetUserToChirpyRedRow
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
		&i.UpdatedAt,
